...
function MJ_get_color (col, row)
	local color = 'blue';
	if (row % 2 == 0 and col % 2 == 0) or (row % 2 == 1 and col % 2 == 1) then
		color = 'white';
	end
	return color;
end
...
function MJ_get_arrow_image (current, new)
	if new.row > current.row then
		if new.col > current.col then
			return "Lua/DK64/arrows/up_right.png";
		elseif new.col == current.col then
			return "Lua/DK64/arrows/up.png";
		elseif new.col < current.col then
			return "Lua/DK64/arrows/up_left.png";
		end
	elseif new.row == current.row then
		if new.col > current.col then
			return "Lua/DK64/arrows/right.png";
		elseif new.col < current.col then
			return "Lua/DK64/arrows/left.png";
		end
	elseif new.row < current.row then
		if new.col > current.col then
			return "Lua/DK64/arrows/down_right.png";
		elseif new.col == current.col then
			return "Lua/DK64/arrows/down.png";
		elseif new.col < current.col then
			return "Lua/DK64/arrows/down_left.png";
		end
	end
	return "Lua/DK64/question-mark.png";
end
		...
		for	col=0,3 do
			...
			if switches_active then
				if (white_pos.row == row and white_pos.col == col) or (blue_pos.row == row and blue_pos.col == col) then
					gui.drawImage("LUA/DK64/switch.png", x, y, MJ_minimap_width, MJ_minimap_height);
				end
			end

			if cur_pos.row == row and cur_pos.col == col then
				gui.drawImage("LUA/DK64/jack_icon.png", x, y, MJ_minimap_width, MJ_minimap_height);
			...
			if kong_position.row == row and kong_position.col == col then
				--gui.drawText(x, y, "K");
				gui.drawImage("LUA/DK64/TinyFaceEdited.png", x, y, MJ_minimap_width, MJ_minimap_height);
			end
		end
	end
