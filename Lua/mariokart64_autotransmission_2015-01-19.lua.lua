...
console.clear()
console.log(math.sqrt (.24^2+40.055^2+1.001^2))
...
MainWindow = forms.newform(655, 800, "Mario Kart 64 Automatic Transmission")
...
--Maneuver dropdown
--forms.label(MainWindow, "Maneuver", 141, 56, 55, 20)
a = { }
...
--HUD Customizations
forms.label(MainWindow, "HUD Settings:  (check to exclude)", 10, 706, 240, 18)
CheckboxHUD_LapPosition = forms.checkbox(MainWindow, "Lap / Position", 10, 721)
CheckboxHUD_Coordinates = forms.checkbox(MainWindow, "Coordinates", 10, 740)
CheckboxHUD_Time = forms.checkbox(MainWindow, "Time", 120, 721)
CheckboxHUD_SpeedState = forms.checkbox(MainWindow, "Speed / State", 120, 740)
...
ItemSelectionDropdown = forms.dropdown(MainWindow, itemOptions, 450, 2, 130, 20)

--Multi-Line Textbox for Input Queue
forms.label(MainWindow, "INPUT QUEUE", 335, 9, 150, 15)
...
while true do
    ...
    if (RecordState == 1) then
      if (isLag == false) then
        forms.settext(GeneratedTextBox, forms.gettext(GeneratedTextBox) .. movie.getinputasmnemonic(emu.framecount()) .. "\r\n")
      end
    elseif (InputQueue ~= nil) then
        ...
        if (ItemBotState == 0) then
            ...
            if (z_delay_timer == 2 or z_delay_timer == 1) then
                -- Make a state here
                savestate.save("ItemBotIterator")
        ...
        if (ItemBotState == 2) then
          item_we_got = memory.read_s8(0x165F5D)
          
          if (item_we_got > 0) then
